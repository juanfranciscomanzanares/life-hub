...
export default function Onboarding() {
  ...
  const cerrar = () => { localStorage.setItem("lh_onboarded", "1"); setVisible(false); };
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 p-4">
      <div className="w-full max-w-sm rounded-2xl border border-slate-700 bg-slate-900 p-8 text-center shadow-2xl">
        <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-indigo-500/15 text-4xl">{paso.emoji}</div>
        <h2 className="mb-2 text-xl font-bold text-slate-100">{paso.titulo}</h2>
